...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const canAccessModule = (module: string): boolean => {
    ...
    // 1. Vérifier les modules spécifiques à l'utilisateur (priorité sur l'entreprise)
    //    null = hérité de l'entreprise, [...] = restriction spécifique à cet utilisateur
    const userModules = (state.user as { modules?: string[] | null }).modules;
    if (userModules !== null && userModules !== undefined) {
      ...
      // 2. Sinon vérifier les modules de l'entreprise
